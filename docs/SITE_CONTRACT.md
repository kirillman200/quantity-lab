...
- Data boundary: Project inputs stay in the browser in URL parameters or localStorage. The app has no account or database. Its separate GA4 property loads only after explicit consent and receives query-free page views, not calculator values or saved projects. Public pages load Google AdSense, and shared URL parameters may be included in advertising requests.
